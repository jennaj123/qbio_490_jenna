...
#rna_se <- load("/Users/JennaJacobs/Documents/USC/qbio490/clinical_maf_rnase_rna.RData")
#rna_counts <- read.csv("/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/brca_rna_count_data.csv", header = TRUE, row.names = 1)
#rna_genes <- read.csv("/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/brca_rna_gene_data.csv", header = TRUE)
#rna_clinical <- read.csv("/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/brca_rna_clincial_data.csv", header = TRUE, row.names = 1)
#rna_clinical$age_category <- ifelse(rna_clinical$age_at_index < 50, "Young", "Old")

remove(rna_clinical, rna_genes, rna_counts)
load("/Users/JennaJacobs/Documents/USC/qbio490/clinical_maf_rnase_rna.RData")
```
...
rna_clinical$definition <- factor(rna_clinical$paper_BRCA_Subtype_PAM50)
head(rna_clinical$ajcc_pathologic_stage)
head(rna_clinical$paper_BRCA_Subtype_PAM50)
...
na_mask2 <- ifelse(is.na(rna_clinical$paper_BRCA_Subtype_PAM50), F, T)
rna_clinical <- rna_clinical[na_mask2, ]
rna_counts <- rna_counts [, na_mask2]
dim(rna_counts)
```
...
load("/Users/JennaJacobs/Downloads/deseq_output.RData")
#deseq didn’t work for me and this file is from Nicole
```
...
```{r}
# write code here
rownames(up_reg_results) <- c(up_reg_results$Gene_Name)
rownames(down_reg_results) <- c(down_reg_results$Gene_Name)
...
Gene ensembl id:ENSG00000198798.5
Log2FoldChange:1.076653		
p-adj value:N/A
More highly expressed in young or old patients:equal
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:

Gene ensembl id:ENSG00000074695.6
Log2FoldChange:0.046376467	
p-adj value:0.4813191	
More highly expressed in young or old patients:equal
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:
...
```{r}
# write code here
write.csv(up_reg_results, "/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/up_reg_results,csv", row.names= TRUE)
write.csv(down_reg_results, "/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/down_reg_results.csv", row.names = TRUE)
write.csv(results, "/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/DESeq2_results.csv", row.names = TRUE)
```
...
```{r}
# create plot here
jpeg("/Users/JennaJacobs/Documents/USC/qbio490/qbio_490_jenna/week8_deseq2_jenna/deseq_volcano.jpg")
EnhancedVolcano(results, lab = rownames(results),x = 'log2FoldChange', y = 'pvalue' , shape = 12, colAlpha = 2)
dev.off()
```
